...
tmux new-window -n jupyter -d "/bin/zsh -i"

sleep 2

SETENV_CMD="conda activate xrt && export PYTHONPATH=\"$PWD:$PWD/../xrt\" && export BASE_DIR=$PWD"

tmux send-keys -t skif-xrt:editor.0 "$SETENV_CMD && nvim" C-m
tmux send-keys -t skif-xrt:editor.1 "htop" C-m
tmux send-keys -t skif-xrt:editor.2 "$SETENV_CMD && clear" C-m
tmux send-keys -t skif-xrt:jupyter.0 "$SETENV_CMD && cd notebooks && jupyter-lab" C-m
