...
sys.path.append(
    r"/nix/store/zms2fc9xb0cgjnbixx3szmqnqwf480f5-python3.12-xrt-1.6.1/lib/python3.12/site-packages"
)
...
crystalSi01 = rmats.CrystalSi(
    t=0.5, geom=r"Laue reflected", name=None, volumetricDiffraction=True, useTT=True
)

powder01 = rmats.Powder(t=1.0, geom=r"Laue reflected")


def build_beamline():
    beamLine = raycing.BeamLine(alignE=90000.0)
    ...
    beamLine.screen01 = rscreens.Screen(bl=beamLine, name=None, center=[0, 20000, 0])
    ...
    beamLine.screen02 = rscreens.Screen(bl=beamLine, name=None, center=[0, 33050, 0])
    ...
    beamLine.screen03 = rscreens.Screen(bl=beamLine, name=None, center=[0, 56000, 25])
    ...
    beamLine.roundBeamStop01 = rapts.RoundBeamStop(bl=beamLine, center=[0, 56050, 25])

    beamLine.screen04 = rscreens.Screen(bl=beamLine, center=[0, 56100, 25])
...
def run_process(beamLine):
    ...
    screen01beamLocal01 = beamLine.screen01.expose(beam=wiggler01beamGlobal01)

    bentLaue2D01beamGlobal01, bentLaue2D01beamLocal01 = beamLine.bentLaue2D01.reflect(
        beam=wiggler01beamGlobal01
    )

    screen02beamLocal01 = beamLine.screen02.expose(beam=bentLaue2D01beamGlobal01)

    bentLaue2D02beamGlobal01, bentLaue2D02beamLocal01 = beamLine.bentLaue2D02.reflect(
        beam=bentLaue2D01beamGlobal01
    )

    screen03beamLocal01 = beamLine.screen03.expose(beam=bentLaue2D02beamGlobal01)
    ...
    screen04beamLocal01 = beamLine.screen04.expose(beam=lauePlate01beamGlobal01)
...
def define_plots():
    ...
    plot02 = xrtplot.XYCPlot(
        ...
        xaxis=xrtplot.XYCAxis(label=r"x", bins=256, ppb=1),
        yaxis=xrtplot.XYCAxis(label=r"y", bins=256, ppb=1),
        caxis=xrtplot.XYCAxis(label=r"energy", unit=r"eV", bins=256, ppb=1),
        aspect=r"auto",
        title=r"CRYSTAL 1",
    )
    plots.append(plot02)
    ...
    plot04 = xrtplot.XYCPlot(
        ...
        xaxis=xrtplot.XYCAxis(label=r"x", bins=512, ppb=1, fwhmFormatStr=r"%.1e"),
        yaxis=xrtplot.XYCAxis(label=r"z", bins=512, ppb=1, fwhmFormatStr=r"%.1e"),
        caxis=xrtplot.XYCAxis(label=r"energy", unit=r"eV", bins=512, ppb=1),
        aspect=r"auto",
        title=r"EXIT",
    )
    plots.append(plot04)
    ...
    plot05 = xrtplot.XYCPlot(
        ...
        xaxis=xrtplot.XYCAxis(label=r"x", bins=512, ppb=1),
        yaxis=xrtplot.XYCAxis(label=r"z", bins=512, ppb=1),
        caxis=xrtplot.XYCAxis(label=r"energy", unit=r"eV", bins=512, ppb=1),
        title=r"DIFFRACTION",
    )
    plots.append(plot05)
...
def main():
    ...
    E0 = 0.5 * (beamLine.wiggler01.eMin + beamLine.wiggler01.eMax)
    beamLine.alignE = E0
    plots = define_plots()
    xrtrun.run_ray_tracing(
        plots=plots, repeats=10000, backend=r"raycing", beamLine=beamLine
    )
