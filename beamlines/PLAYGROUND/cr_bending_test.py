...
class BENDY(raycing.BeamLine):
    def __init__(self):
        ...
        self.Cr2Monitor = rscreens.Screen(
            ...
            center=[0, croc_crl_distance - 10, monochromator_z_offset],
        )
...
def run_process(bl: BENDY):
    ...
    beam_mon2 = bl.Cr2Monitor.expose(beam=beam_mono_c2_global)
...
def align_energy(bl: BENDY, en, d_en):
    ...
    bl.alignE = en
    if not mono:
        bl.SuperCWiggler.eMin = en * (1.0 - d_en)
        bl.SuperCWiggler.eMax = en * (1.0 + d_en)
    else:
    ...
    bl.MonochromatorCr1.center = [0.0, monochromator_distance, 0.0]
    ...
    bl.Cr1Monitor.center = [
        0.0,
        monochromator_distance + 0.5 * monochromator_z_offset / np.tan(2.0 * theta0),
...
def upd_plots(plts, bl: BENDY, en):
    ...
    for plot in plts:
        ...
        plot.caxis.limits = [en * (1.0 - de_over_e), en * (1.0 + de_over_e)]

        # By default no plots need offset
        plot.xaxis.offset = 0.0
        plot.yaxis.offset = 0.0
        ...
        if plot.title in ("C1-XprZpr", "C1-ZZpr"):
            plot.yaxis.offset = bl.Cr1Monitor.center[2] / (
                bl.Cr1Monitor.center[1] - bl.MonochromatorCr1.center[1]
            )

        # Setting z^{\prime} limits
        if "Zpr" in plot.title:
            plot.yaxis.limits = [
                plot.yaxis.offset - plot_zpr_lim,
                plot.yaxis.offset + plot_zpr_lim,
            ]
        ...
        if "XZ" in plot.title:
            plot.yaxis.limits = [
                plot.yaxis.offset - plot_z_lim,
                plot.yaxis.offset + plot_z_lim,
            ]

        if "ZZpr" in plot.title:
            plot.xaxis.limits = [
                plot.xaxis.offset - plot_z_lim,
                plot.xaxis.offset + plot_z_lim,
            ]
...
def set_de_over_e(radius):
    ...
    if radius < 4.0e3:
        de_over_e = 1.5e4 / 70.0e3
    elif 4.0e3 <= radius < 8.0e3:
        de_over_e = 4.5e3 / 70.0e3
    elif 8.0e3 <= radius < 15.0e3:
        de_over_e = 1.5e3 / 70.0e3
    elif 15.0e3 <= radius < 21.0e3:
        de_over_e = 7.5e2 / 70.0e3
    elif 21.0e3 <= radius < 28.0e3:
        de_over_e = 3.0e2 / 70.0e3
    elif 28.0e3 <= radius < 30.0e3:
        de_over_e = 2.0e2 / 70.0e3
    elif 30.0e3 <= radius < 39.0e3:
        de_over_e = 7.5e1 / 70.0e3
    elif 39.0e3 <= radius < 52.0e3:
        de_over_e = 1.5e2 / 70.0e3
    elif 52.0e3 <= radius < 100.0e3:
        de_over_e = 3.0e2 / 70.0e3
    elif 100.0e3 <= radius:
        de_over_e = 6.0e2 / 70.0e3

    if 3.0e4 <= radius:
        plot_zpr_lim = 1e-3
    elif 1.5e4 <= radius < 3.0e4:
        plot_zpr_lim = 2e-3
    elif radius < 1.5e4:
        plot_zpr_lim = 7e-3
...
def scan1(plts, bl: BENDY):
    ...
    for r in np.arange(2.0, 6.0, 0.1) * 1e3:
