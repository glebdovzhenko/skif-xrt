...
class FocusLocator:
            ...
            [
                XYCPlot(
                    ...
                    persistentName=os.path.join(
                        self._subdir, self.plot_fmt % screen.center[1] + ".pickle"
                    ),
                    saveName=os.path.join(
                        self._subdir, self.plot_fmt % screen.center[1] + ".png"
                    ),
                    aspect="auto",
                    xaxis=XYCAxis(label="$x$", unit="mm", data=get_x),
                    yaxis=XYCAxis(
                        label="$z$", unit="mm", data=get_z, limits=[-0.5, 0.5]
                    ),
                )
    ...
    def parabolic_fit(self, bl_, cutoff=0.1):
        pos, x_size, z_size = [], [], []
        flux = None
        for f_name in (
            os.path.join(self._subdir, self.plot_fmt % screen.center[1] + ".pickle")
        ...
        fig.savefig(
            os.path.join(
                self.data_dir,
                "fdist-%.03fm-%.03fm.png" % (np.min(pos * 1e-3), np.max(pos * 1e-3)),
            )
        )
    ...
    def beamline(self, klass):
        def flscreens_reset(self_, y_min, y_max, n):
            ...
            self_._FLScreens = [
                Screen(
                    bl=self_, name=self.screen_fmt % yy, center=[self.x0, yy, self.z0]
                )
    ...
    def run_process(self, fn):
        def fn_(bl_):
            ...
            for iscreen, screen in enumerate(bl_._FLScreens):
                outDict_[self.beam_fmt % iscreen] = screen.expose(
                    beam=outDict_[self.beam_name]
                )
            return outDict_
    ...
    def gnrtr(
        self, y_min=None, y_max=None, nscreens=None, niterations=None, cutoff=None
    ):
